Program TimingAccuracyDH
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
!	This program will test the accuracy of the spherical harmonic Driscoll and 
!	Healy tranformation routines by expanding a field in the space domain, transforming this
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	do lmax=100, maxdeg, 100
		...
		if (sampling == 1) then
			call MakeGridDH(griddh(1:n, 1:n), n, cilm(1:2,1:lmax+1, 1:lmax+1), lmax, sampling=sampling)
		else
			call MakeGridDH(griddh(1:n, 1:2*n), n, cilm(1:2,1:lmax+1, 1:lmax+1), lmax, sampling=sampling)
		endif
		...
		if (sampling == 1) then
			call SHExpandDH(griddh(1:n, 1:n), n, cilm2(1:2, 1:lmax+1, 1:lmax+1), lmaxout, sampling=sampling)
		else
			call SHExpandDH(griddh(1:n, 1:2*n), n, cilm2(1:2, 1:lmax+1, 1:lmax+1), lmaxout, sampling=sampling)
		endif
