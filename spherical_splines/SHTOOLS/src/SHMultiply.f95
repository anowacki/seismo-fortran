...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	integer :: 	lmaxout, phase, mnorm, astat(3)
	real*8 ::	zero(lmax1+lmax2+1), w(lmax1+lmax2+1)
	...
	allocate(grid1glq(lmax1+lmax2+1, 2*(lmax1+lmax2)+1), stat = astat(1))
	allocate(grid2glq(lmax1+lmax2+1, 2*(lmax1+lmax2)+1), stat = astat(2))
	...
	if (present(precomp)) then
		if (precomp == 0) then
			...
			grid1glq = grid1glq*grid2glq
	
			call SHExpandGLQ(shout, lmaxout, grid1glq, w, zero = zero, csphase = phase, norm = mnorm)
			
		else
			allocate ( plx(lmax1+lmax2+1, (lmax1+lmax2+1)*(lmax1+lmax2+2)/2 ), stat=astat(3) )
			if (astat(3) /=0) then 
				print*, "Error --- SHMultiply"
				print*, "Problem allocating array PLX", astat(3)
				stop
			endif
