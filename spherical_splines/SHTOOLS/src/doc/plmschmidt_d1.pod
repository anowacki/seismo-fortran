...
An array of Schmidt-normalized associated Legendre functions up to degree LMAX. The index corresponds to L*(L+1)/2 + M + 1, which can be calculated by a call to I<PlmIndex>.
	
=item DP

(output) REAL*8, DIMENSION ((LMAX+1)*(LMAX+2)/2)

An array of the first derivatives of the Schmidt-normalized associated Legendre functions up to degree LMAX. The index corresponds to L*(L+1)/2 + M + 1, which can be calculated by a call to I<PlmIndex>.
...
plbar(1), plbar_d1(1), plmbar(1), plmbar_d1(1), plon(1), plon_d1(1), plmon(1), plmon_d1(1), plschmidt(1), plmschmidt(1), plmschmidt_d1(1), plegendre(1), plegendre_d1(1), plegendrea(1), plegendrea_d1(1), plmindex(1)
