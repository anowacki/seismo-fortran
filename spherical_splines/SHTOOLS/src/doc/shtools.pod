...
directly after the routine name, and before an IMPLICIT NONE statement.  When compiling your code, it will be necessary to link to the SHTOOLS library and pre-compiled module files. For this purpose, it may be useful to define the environment variables SHTOOLSMODPATH and SHTOOLSLIBPATH in your login file by using

   setenv SHTOOLSMODPATH = "shtoolslocation"/SHTOOLS/modules
   setenv SHTOOLSLIBPATH = "shtoolslocation"/SHTOOLS/lib
   
where "shtoolslocation" is the root directory of the SHTOOLS directory, normally /usr/local. In addition, some routines require linking to the fast Fourier transform package FFTW, and linear algebra packages LAPACK and BLAS.
