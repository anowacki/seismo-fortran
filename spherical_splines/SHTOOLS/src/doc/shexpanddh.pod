...
(input) REAL*8, DIMENSION (N, N) I<or> (N, 2*N)
...
(output) REAL*8, DIMENSION (2, N/2, N/2) I<or> S<(2, LMAX_CALC+1, LMAX_CALC+1)> 

The real spherical harmonic coefficients of the function. These will be exact if the function is bandlimited to degree LMAX = N/2 - 1. The coefficients C1lm and C2lm refer to the "cosine" (Clm) and "sine" (Slm) coefficients, respectively, with Clm = CILM(1, l+1, m+1) and Slm = CILM(2, l+1, m+1). Alternatively, C1lm and C2lm correspond to the positive and negative order coefficients, respectively.
...
makegriddh(1), dhaj(1), shexpandglq(1), makegridglq(1), SHExpandLSQ(1)
