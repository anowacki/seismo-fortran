...
!	Notes:
!	
!	1.	The employed normalization is the "Schmidt semi-normalized convention." The integral of
!		(plm*cos(m theta))**2 or (plm*sin (m theta))**2 over all space is 4 pi/(2L+1).
!	2.	The integral of plm**2 over (-1,1) is 2 * (2 - delta(0,m)) / (2L+1). If CNORM=1, then
!		this is equal to 2/(2l+1).
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!	
	...
	if (lmax /= lmax_old) then
		...
		! 		PlmSchmidt(l,m) = x*f1(l,m)*PlmSchmidt(l-1,m) - PlmSchmidt(l-2,m)*f2(l,m)
		!		k = l*(l+1)/2 + m + 1
