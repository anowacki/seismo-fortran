subroutine Hilm(cilm, ba, grid, lmax, nmax, mass, r0, rho, gridtype, w, plx, zero, filter_type, filter_deg, lmax_calc)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
...
!
!	Dependencies:		NGLQSH, SHExpandGLQ, SHExpandDH, wl, wl_curv
!
...
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	use SHTOOLS, only: NGLQSH, SHExpandGLQ, SHExpandDH, Wl, WlCurv
	...
	if(size(grid(1,:)) < nlong .or. size(grid(:,1)) <  nlat) then
		print*, "Error --- Hilm"
		print*, "grid must be dimensioned as (NLAT, NLONG) where"
		print*, "NLAT = ", nlat
		print*, "NLONG = ", nlong
	...
	if (present(w)) then
		if (gridtype /= 1) then
			print*, "Error --- Hilm"
			print*, "W can only be present when GRIDTYPE is 1 (Gauss-Legendre Quadratue)"
	...
	if (present(zero)) then
		if (gridtype /= 1) then
			print*, "Error --- Hilm"
			print*, "ZERO can only be present when GRIDTYPE is 1 (Gauss-Legendre Quadratue)"
			print*, "GRIDTYPE = ", gridtype
			stop
	...
	if (present(plx)) then
		if (gridtype /= 1) then
			print*, "Error --- Hilm"
			print*, "PLX can only be present when GRIDTYPE is 1 (Gauss-Legendre Quadratue)"
			print*, "GRIDTYPE = ", gridtype
			stop
	...
	do n=2, nmax
		...
		elseif (gridtype == 2) then
			call SHExpandDH((grid2(1:nlat,1:nlong)/d)**n, nlat, cilmn, lmax_out, norm = 1, &
				sampling = 1, csphase = 1, lmax_calc = lmax_calc2)
		elseif (gridtype == 3) then
			call SHExpandDH((grid2(1:nlat,1:nlong)/d)**n, nlat, cilmn, lmax_out, norm = 1, &
				sampling = 2, csphase = 1, lmax_calc = lmax_calc2)
