...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	call dfftw_plan_dft_c2r_1d(plan, nlong, coef, grid, FFTW_MEASURE)	! create generic plan
	...
	if (present(plx)) then
		...
		do i=1, (nlat+1)/2
			...
			if ( i==(nlat+1)/2 .and. mod(nlat,2) /=0) then		! This latitude is the equator; z=0, u=1
				...
				coef0 = coef0 + cilm(1,1,1) * pm2
				coef0s = coef0s + cilm(1,1,1) * pm2 	! symsign is always 1 for l=m=0
				
				k = 2
				pm1 =  f1(k) * z * pm2
				coef0 = coef0 + cilm(1,2,1) * pm1
				coef0s = coef0s + cilm(1,2,1) * pm1 * symsign(k)
				...
				do l=2, lmax_comp, 1
					...
					coef0 = coef0 + cilm(1,l1,1) * p
					coef0s = coef0s + cilm(1,l1,1) * p * symsign(k)
					pm2 = pm1
					pm1 = p
				enddo
				...
				do m = 1, lmax_comp-1, 1
					...
					do l = m+2, lmax_comp, 1
						...
						coefs(m1) = coefs(m1) + dcmplx(cilm(1,l1,m1), - cilm(2,l1,m1)) * p * symsign(k)
					enddo
